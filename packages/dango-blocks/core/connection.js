...
Blockly.Connection.prototype.checkType_ = function(otherConnection) {
  if (!this.check_ || !otherConnection.check_) {
    ...
    // Lambda blocks cannot be nested into normal input.
    if (this.check_ && this.check_.indexOf('Lambda') != -1 && !otherConnection.check_) return false;
    if (!this.check_ && otherConnection && otherConnection.check_.indexOf('Lambda') !== -1) return false;
...
Blockly.Connection.prototype.getOutputShape = function() {
  ...
  if (this.check_.indexOf('Lambda') !== -1) {
    return Blockly.OUTPUT_SHAPE_LAMBDA;
  }
  return Blockly.OUTPUT_SHAPE_ROUND;
};
