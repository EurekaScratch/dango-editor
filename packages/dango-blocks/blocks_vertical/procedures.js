...
Blockly.ScratchBlocks.ProcedureUtils.createAllInputs_ = function(connectionMap) {
  // Split the proc into components, by %n, %b, %l, and %s (ignoring escaped).
  ...
  for (var i = 0, component; component = procComponents[i]; i++) {
    ...
    if (component.substring(0, 1) == '%') {
      ...
      if (!(argumentType == 'n' || argumentType == 'b' || argumentType == 's' || argumentType == 'l')) {
        throw new Error(
...
Blockly.ScratchBlocks.ProcedureUtils.createArgumentReporter_ = function(
  ...
  } else if (argumentType == 'l') {
    var blockType = 'argument_lambda';
  } else {
...
Blockly.ScratchBlocks.ProcedureUtils.populateArgumentOnCaller_ = function(type,
  ...
  if (connectionMap && oldBlock) {
    ...
    if (type != 'b' && type != 'l' && this.generateShadows_) {
...
Blockly.ScratchBlocks.ProcedureUtils.checkOldTypeMatches_ = function(oldBlock,
  ...
  if (type == 'l' && oldBlock.type == 'argument_lambda') {
    return true;
  }
  return false;
};
...
Blockly.ScratchBlocks.ProcedureUtils.createArgumentEditor_ = function(
  ...
  try {
    if (argumentType == 'n' || argumentType == 's') {
      ...
      var newBlock = this.workspace.newBlock('argument_editor_lambda');
    } else {
  ...
  for (var i = 0; i < this.inputList.length; i++) {
    ...
    if (input.type == Blockly.DUMMY_INPUT) {
      ...
      if (target.type == 'argument_editor_boolean') {
        this.procCode_ += '%b';
      } else if (target.type == 'argument_editor_lambda') {
        this.procCode_ += '%l';
      } else {
        this.procCode_ += '%s';
      }
...
/**
 * Externally-visible function to add a lambda argument to the procedure
 * declaration.
 * @public
 */
Blockly.ScratchBlocks.ProcedureUtils.addLambdaExternal = function () {
  Blockly.WidgetDiv.hide(true);
  this.procCode_ = this.procCode_ + ' %l';
  this.displayNames_.push('lambda');
  this.argumentIds_.push(Blockly.utils.genUid());
...
Blockly.Blocks['procedures_declaration'] = {
  ...
  addLambdaExternal: Blockly.ScratchBlocks.ProcedureUtils.addLambdaExternal,
...
Blockly.Blocks['argument_lambda'] = {
  init: function () {
    this.jsonInit({
      "message0": " %1",
      "args0": [
...
Blockly.Blocks['argument_editor_lambda'] = {
  init: function () {
    this.jsonInit({
      "message0": " %1",
      "args0": [
...
Blockly.Blocks['procedures_return'] = {
  ...
  onchange: function(_event) {
    ...
    if (this.getRootBlock().type == 'procedures_definition_return'
      || this.getRootBlock().type == 'operators_lambda') {
      if (!this.isInFlyout) {
