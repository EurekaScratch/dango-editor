...
import SecurityManager from './extension-support/tw-security-manager';
import log from './util/log';
import Thread from './engine/thread';
import Target from './engine/target';
import MathUtil from './util/math-util';
import Runtime, {CompilerOptions, RuntimeOptions, AddonOptions} from './engine/runtime';
import StringUtil from './util/string-util';
import * as formatMessage from 'format-message';
import Variable, { VariableValue } from './engine/variable';
...
// @ts-expect-error TS(7016): Could not find a declaration file for module 'scra... Remove this comment to see the full error message
...
import CCExtensionAPI from './extension-support/cc-extension-api';

/**
 * Indicates the type is dependent on the existence of a renderer.
 */
export type IfRenderer<HasRenderer, NoRenderer> = HasRenderer;

/**
 * Indicates the type is dependent on the existence of an audio engine.
 */
export type IfAudioEngine<HasAudioEngine, NoAudioEngine> = HasAudioEngine;

/**
 * Indicates thte type is dependent on the existence of an attached storage.
 */
export type IfStorage<HasStorage, NoStorage> = HasStorage;

/**
 * Indicates the type is dependent of whether the VM is attached to scratch-gui.
 */
export type IfGui<HasGui, NoGui> = HasGui;

export type ScratchCompatibleValue = string | boolean | number;


type ZipType = 'base64' | 'binarystring' | 'array' | 'uint8array' | 'arraybuffer' | 'blob' | 'nodebuffer';
...
const createRuntimeService = (runtime: Runtime) => {
    return {
        _refreshExtensionPrimitives: runtime._refreshExtensionPrimitives.bind(runtime),
        _registerExtensionPrimitives: runtime._registerExtensionPrimitives.bind(runtime)
    };
};
/**
...
class VirtualMachine extends EventEmitter {
    ...
    constructor () {
        ...
        this.runtime.on(Runtime.TARGETS_UPDATE, (emitProjectChanged: any) => {
        ...
        this.runtime.on(Runtime.BLOCK_DRAG_END, (blocks: any, topBlockId: any) => {
            this.emit(Runtime.BLOCK_DRAG_END, blocks, topBlockId);
        });
        this.runtime.on(Runtime.EXTENSION_ADDED, (categoryInfo: any) => {
            this.emit(Runtime.EXTENSION_ADDED, categoryInfo);
        });
        this.runtime.on(Runtime.EXTENSION_FIELD_ADDED, (fieldName: any, fieldImplementation: any) => {
        ...
        this.runtime.on(Runtime.PERIPHERAL_CONNECTION_LOST_ERROR, (data: any) => this.emit(Runtime.PERIPHERAL_CONNECTION_LOST_ERROR, data));
        ...
        this.runtime.on(Runtime.COMPILER_OPTIONS_CHANGED, (compilerOptions: any) => {
    ...
    setRuntimeOptions (runtimeOptions: RuntimeOptions) {
    ...
    getPlaygroundData () {
        ...
        const threadData = this.runtime.threads.filter((thread: Thread) => thread.target === instance.editingTarget);
     ...
     */
    postIOData (device: string, data: object) {
     ...
     */
    loadProject (input: ArrayBufferView | ArrayBuffer | string | object) {
        ...
        const validationPromise = new Promise((resolve, reject) => {
            ...
            validate(input as ArrayBuffer | string , false, (error, res) => {
                if (error) {
                    return reject(error);
                }
                resolve(res);
     ...
     * @param {string | number} id - the ID of the project to download, as a string.
     */
    downloadProjectId (id: string | number) {
     ...
     */
    saveProjectSb3 (type: JSZip.OutputType) {
     ...
     */
    saveProjectSb3Stream (type: JSZip.OutputType) {
     ...
     */
    get assets () {
        return this.runtime.targets.reduce((acc: any, target: RenderedTarget) => (acc
            .concat(target.sprite.sounds.map((sound: any) => sound.asset))
            .concat(target.sprite.costumes.map((costume: Costume) => costume.asset))), []);
    }
    _addFileDescsToZip (fileDescs: any, zip: any) {
     ...
     */
    exportSprite (targetId: string, optZipType: ZipType) {
     ...
     * @param {SerializationOptions?} serializationOptions Options to pass to the serializer
     * @return {string} Serialized state of the runtime.
     */
    toJSON (optTargetId?: string, serializationOptions?: SerializationOptions) {
     ...
     * @param {ArrayBufferView | ArrayBuffer | string | object} json JSON string representing a project.
     * @returns {Promise} Promise that resolves after the project has loaded
     */
    fromJSON (json: ArrayBufferView | ArrayBuffer | string | object) {
     ...
     * @returns {Promise} Promise that resolves after the project has loaded
     */
    deserializeProject (projectJSON: SB3JSON, zip?: JSZip) {
        ...
        return deserializePromise()
            .then(({targets, extensions}) => {
                ...
                return this.installTargets(targets, extensions as ImportedExtensionsInfo, true);
            });
    }
    /**
     ...
     */
    async _loadExtensions (extensionIDs: Set<unknown>, extensionURLs = new Map()) {
     ...
     */
    async installTargets (targets: RenderedTarget[], extensions: ImportedExtensionsInfo, wholeProject: any) {
        ...
        return this._loadExtensions(extensions.extensionIDs, extensions.extensionURLs).then(() => {
            ...
            this.runtime.executableTargets.sort((a: any, b: any) => a.layerOrder - b.layerOrder);
            targets.forEach((target: any) => {
     ...
     * @return {!Promise} Promise that resolves after targets are installed.
     */
    addSprite (input: ArrayBufferView | ArrayBuffer | string | object) {
        ...
        const validationPromise = new Promise((resolve, reject) => {
            // @ts-expect-error TS(7022): 'validate' implicitly has type 'any' because it do... Remove this comment to see the full error message
            ...
            // an entire project
            validate(input, true, (error: any, res: any) => {
                if (error) {
                    return reject(error);
                }
                resolve(res);
     ...
     */
    _addSprite2 (sprite: any, zip: any) {
        // Validate & parse
        // @ts-expect-error TS(7022): 'sb2' implicitly has type 'any' because it does no... Remove this comment to see the full error message
        const sb2 = sb2;
        return sb2.deserialize(sprite, this.runtime, true, zip)
            .then(({
                targets,
                extensions
            }: any) => this.installTargets(targets, extensions, false));
    }
    /**
     ...
     * @returns {Promise} Promise that resolves after the sprite is added
     */
    _addSprite3 (sprite: any, zip: any) {
        // Validate & parse
        // @ts-expect-error TS(7022): 'sb3' implicitly has type 'any' because it does no... Remove this comment to see the full error message
        const sb3 = sb3;
        return sb3
     ...
     */
    duplicateCostume (costumeIndex: number) {
     ...
     */
    duplicateSound (soundIndex: number) {
     ...
     * @returns {?Promise} - a promise that resolves when the sound has been decoded and added
     */
    addSound (soundObject: Sound, optTargetId: string) {
        const target = optTargetId ? this.runtime.getTargetById(optTargetId) :
            this.editingTarget;
        if (target) {
     ...
     */
    getSoundBuffer (soundIndex: number) {
     ...
     */
    updateSoundBuffer (soundIndex: number, newBuffer: AudioBuffer, soundEncoding?: ArrayBuffer) {
        const sound = this.editingTarget?.sprite.sounds[soundIndex];
        if (sound && sound.broken) {
            delete sound.broken;
        }
        const id = sound ? sound.soundId : null;
        if (id && this.editingTarget && this.runtime && this.runtime.audioEngine) {
     ...
     * @returns {Uint8Array}
     */
    getExportedCostume (costumeObject: Costume) {
     ...
     */
    getExportedCostumeBase64 (costumeObject: Costume) {
     ...
     */
    updateBitmap (costumeIndex: number, bitmap: ImageData, rotationCenterX: number, rotationCenterY: number, bitmapResolution: number) {
    ...
    _updateBitmap (costume: any, bitmap: any, rotationCenterX: any, rotationCenterY: any, bitmapResolution: any) {
    ...
    _updateSvg (costume: any, svg: any, rotationCenterX: any, rotationCenterY: any) {
        if (costume && costume.broken) {
            delete costume.broken;
        }
        if (costume && this.runtime && this.runtime.renderer) {
     ...
     */
    addBackdrop (md5ext: string, backdropObject: Costume) {
        // @ts-expect-error TS(2554): Expected 4 arguments, but got 3.
        ...
        if (target) {
            ...
            if (newName && RESERVED_NAMES.indexOf(newName) === -1) {
                const names = this.runtime.targets
                    .filter((runtimeTarget: any) => runtimeTarget.isSprite() && runtimeTarget.id !== target.id)
    ...
    deleteSprite (targetId: string) {
        const target = this.runtime.getTargetById(targetId);
        if (target) {
            const targetIndexBeforeDelete = this.runtime.targets.map((t: any) => t.id).indexOf(target.id);
            if (!target.isSprite()) {
     ...
     */
    duplicateSprite (targetId: string) {
        const target = this.runtime.getTargetById(targetId);
        if (!target) {
            throw new Error('No target with the provided id.');
     ...
     */
    setLocale (locale: string, messages: {[key: string]: string | formatMessage.Translation}) {
     ...
     */
    shareBlocksToTarget (blocks: any, targetId: string, optFromTargetId: string) {
        ...
        const extensionIDs = new Set(copiedBlocks
            .map((b: any) => sb3.getExtensionIdForOpcode(b.opcode))
     ...
     */
    shareCostumeToTarget (costumeIndex: number, targetId: string) {
        const originalCostume = this.editingTarget?.getCostumes()[costumeIndex];
        const clone = Object.assign({}, originalCostume);
        const md5ext = `${clone.assetId}.${clone.dataFormat}`;
        // @ts-expect-error TS(2554): Expected 4 arguments, but got 3.
     ...
     */
    shareSoundToTarget (soundIndex: number, targetId: string) {
        const originalSound = this.editingTarget?.getSounds()[soundIndex];
        const clone = Object.assign({}, originalSound);
        const target = this.runtime.getTargetById(targetId);
        ...
        const getTargetListLazily = () => {
            if (!lazyTargetList) {
                lazyTargetList = this.runtime.targets
                    .filter(
                        // Don't report clones.
                        (target: any) => !target.hasOwnProperty('isOriginal') || target.isOriginal).map((target: any) => target.toJSON());
            }
     ...
     */
    setVariableValue (targetId: string, variableId: string, value: VariableValue) {
        const target = this.runtime.getTargetById(targetId);
        if (target) {
...
// Export type for doc generation
export {
    Runtime,
    RuntimeOptions,
    CompilerOptions,
...
export * as ExtensionDeclaration from './extension-support/extension-metadata';
