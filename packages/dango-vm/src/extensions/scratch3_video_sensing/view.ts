...
class VideoMotionView {
    ...
    motion: any;
    output: any;
    constructor (motion: any, output = OUTPUT.XYT) {
        /**
     ...
     */
    _eachAddress (xStart: any, yStart: any, xStop: any, yStop: any, fn: any) {
     ...
     */
    _eachCell (xStart: any, yStart: any, xStop: any, yStop: any, xStep: any, yStep: any, fn: any) {
        const xStep2 = (xStep / 2) | 0;
        const yStep2 = (yStep / 2) | 0;
        for (let i = yStart; i < yStop; i += yStep) {
            for (let j = xStart; j < xStop; j += xStep) {
                fn(
                    (_fn: any) => this._eachAddress(j - xStep2 - 1, i - yStep2 - 1, j + xStep2, i + yStep2, _fn),
                    j - xStep2 - 1,
                    i - yStep2 - 1,
                    j + xStep2,
                    i + yStep2
    ...
    draw () {
        ...
        if (this.output === OUTPUT.INPUT) {
            ...
            this._eachAddress(1, 1, WIDTH - 1, HEIGHT - 1, (address: any) => {
        ...
        if (this.output === OUTPUT.XYT_CELL) {
            ...
            this._eachCell(WINSIZE + 1, WINSIZE + 1, wmax, hmax, winStep, winStep, (eachAddress: any) => {
                let C1 = 0;
                let C2 = 0;
                let n = 0;
        ...
        if (this.output === OUTPUT.AB_CELL) {
            ...
            this._eachAddress(1, 1, WIDTH - 1, HEIGHT - 1, (address: any) => {
                const {A2, A1B2, B1, C2, C1} = this._components((fn: any) => fn(address));
