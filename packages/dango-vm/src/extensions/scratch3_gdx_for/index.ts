...
// @ts-expect-error TS(7016): Could not find a declaration file for module '@ver... Remove this comment to see the full error message
import godirect from '@vernier/godirect/dist/godirect.min.umd';
...
class GdxFor {
    ...
    _onConnect () {
        ...
        godirect.createDevice(adapter, {open: true, startMeasurements: false}).then((device: any) => {
            // Setup device
            this._device = device;
            this._device.keepValues = false; // todo: possibly remove after updating Vernier godirect module
            // Enable sensors
            this._device.sensors.forEach((sensor: any) => {
                sensor.setEnabled(true);
            });
            // Set sensor value-update behavior
            this._device.on('measurements-started', () => {
                const enabledSensors = this._device.sensors.filter((s: any) => s.enabled);
                enabledSensors.forEach((sensor: any) => {
                    sensor.on('value-changed', (s: any) => {
     ...
     * @private
     */
    _onSensorValueChanged (sensor: any) {
 ...
 */
class Scratch3GdxForBlocks implements StandardScratchExtensionClass {
    _facingDown: any;
    _facingUp: any;
    _peripheral: any;
    runtime: any;
    ...
    whenForcePushedOrPulled (args: any) {
