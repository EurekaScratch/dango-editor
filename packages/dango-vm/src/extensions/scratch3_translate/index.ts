 ...
 * @constructor
 */
class Scratch3TranslateBlocks implements StandardScratchExtensionClass {
    _lastLangTranslated: any;
    _lastTextTranslated: any;
    _randomLanguageCode: any;
    _supportedLanguages: any;
    _translateResult: any;
    _viewerLanguageCode: any;
    constructor () {
        /**
     ...
     * @private
     */
    _getSupportedLanguages (code: any) {
        return languageNames.menuMap[code].map((entry: any) => {
    ...
    getViewerLanguage () {
        ...
        let langNameObj = names.find((obj: any) => obj.code === this._viewerLanguageCode);
        ...
        if (!langNameObj && languageNames.scratchToGoogleMap[this._viewerLanguageCode]) {
            ...
            langNameObj = names.find((obj: any) => obj.code === lookupCode);
        }
    ...
    getViewerLanguageCode () {
        ...
        const languageCode = viewerLanguages.reduce((acc, lang) => {
            ...
            // @ts-expect-error TS(2339): Property 'toLowerCase' does not exist on type 'Loc... Remove this comment to see the full error message
     ...
     */
    getLanguageCodeFromArg (arg: any) {
