...
class EV3Motor {
     ...
     */
    constructor (parent: any, index: any, type: any) {
        /**
...
class EV3 {
    ...
    isButtonPressed (port: any) {
        return this._sensors.buttons[port] === 1;
    }
    beep (freq: any, time: any) {
     ...
     */
    send (message: any, useLimiter = true) {
        if (!this.isConnected()) {
     ...
     */
    generateCommand (type: any, byteCommands: any, allocation = 0) {
        // Header (Bytes 0 - 6)
        let command = [];
...
class Scratch3Ev3Blocks implements StandardScratchExtensionClass {
    _peripheral: any;
    runtime: any;
    /**
    ...
    motorTurnCounterClockwise (args: any) {
        const port = Cast.toNumber(args.PORT);
        let time = Cast.toNumber(args.TIME) * 1000;
        time = MathUtil.clamp(time, 0, 15000);
        return new Promise(resolve => {
    ...
    _playNoteForPicker (note: any, category: any) {
