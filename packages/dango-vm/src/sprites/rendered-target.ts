...
class RenderedTarget extends Target {
    ...
    get audioPlayer () {
        ...
        const audioPlayerProxy = {
            playSound: (soundId: any) => bank.play(this, soundId)
        };
     ...
     */
    setEffect (effectName: any, value: any) {
     ...
     */
    addCostume (costumeObject: any, index?: any) {
    ...
    renameCostume (costumeIndex: any, newName: any) {
        const usedNames = this.sprite.costumes
            .filter((costume: any, index: any) => costumeIndex !== index)
     ...
     * @param {?int} index Index at which to add costume
     */
    addSound (soundObject: any, index?: any) {
        const usedNames = this.sprite.sounds.map((sound: any) => sound.name);
    ...
    renameSound (soundIndex: any, newName: any) {
        const usedNames = this.sprite.sounds
            .filter((sound: any, index: any) => soundIndex !== index)
     ...
     */
    setRotationStyle (rotationStyle: any) {
     ...
     */
    getCostumeIndexByName (costumeName: any) {
     ...
     */
    reorderCostume (costumeIndex: any, newIndex: any) {
     ...
     * @returns {boolean} If a change occurred (i.e. if the indices do not match)
     */
    reorderSound (soundIndex: any, newIndex: any) {
     ...
     */
    isTouchingObject (requestedObject: any) {
     ...
     */
    isTouchingSprite (spriteName: any) {
        ...
        const drawableCandidates = firstClone.sprite.clones.filter((clone: any) => !clone.dragging)
            .map((clone: any) => clone.drawableID);
     ...
     */
    colorIsTouchingColor (targetRgb: any, maskRgb: any) {
        if (this.renderer) {
     ...
     */
    keepInFence (newX: any, newY: any, optFence: any) {
        let fence = optFence;
        if (!fence) {
            fence = {
     ...
     */
    duplicate () {
        return this.sprite.duplicate().then((newSprite: any) => {
     ...
     */
    // @ts-expect-error TS(2416): Property 'postSpriteInfo' in type 'RenderedTarget'... Remove this comment to see the full error message
    postSpriteInfo (data: any) {
