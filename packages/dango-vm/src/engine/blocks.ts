...
class Blocks {
    ...
    forceNoGlow: any;
    runtime: any;
    constructor (runtime: Runtime, optNoGlow?: boolean) {
     ...
     */
    getCachedCompileResult (blockId: any) {
     ...
     */
    cacheCompileResult (blockId: any, value: any) {
     ...
     */
    cacheCompileError (blockId: any, error: any) {
        this._cache.compiledScripts[blockId] = {
            success: false,
            value: error
        };
     ...
     */
    getBranch (id: any, branchNum: any) {
        const block = this._blocks[id];
     ...
     */
    getProcedureDefinition (name: any) {
     ...
     */
    getProcedureParamNamesAndIds (name: any) {
     ...
     * @return {?Array.<string>} List of param names for a procedure.
     */
    getProcedureParamNamesIdsAndDefaults (name: any) {
    ...
    blocklyListen (e: any) {
        ...
        case 'var_create':
            ...
            if (e.isLocal && editingTarget && !editingTarget.isStage && !e.isCloud) {
                ...
                const allTargets = this.runtime.targets.filter((t: any) => t.isOriginal);
     ...
     */
    getAllVariableAndListReferences (optBlocks: any, optIncludeBroadcast: any) {
     ...
     */
    updateBlocksAfterVarRename (varId: any, newName: any) {
     ...
     */
    updateTargetSpecificBlocks (isStage: any) {
        const blocks = this._blocks;
        for (const blockId in blocks) {
     ...
     * 'backdrop'.
     */
    updateAssetName (oldName: any, newName: any, assetType: any) {
        let getAssetField;
     ...
     */
    updateSensingOfReference (oldName: any, newName: any, targetName: any) {
        const blocks = this._blocks;
     ...
     */
    toXML (comments: any) {
        return this._scripts.map((script: any) => this.blockToXML(script, comments)).join();
    }
    /**
     ...
     */
    blockToXML (blockId: any, comments: any) {
        const block = this._blocks[blockId];
        ...
        for (const input in block.inputs) {
            if (!block.inputs.hasOwnProperty(input)) {
                continue;
            }
        ...
        for (const field in block.fields) {
            if (!block.fields.hasOwnProperty(field)) {
                continue;
            }
     ...
     */
    _getCustomBlockInternal (defineBlock: any) {
