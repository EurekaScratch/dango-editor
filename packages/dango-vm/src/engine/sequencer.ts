...
class Sequencer {
     ...
     */
    stepToBranch (thread: any, branchNum: any, isLoop: any) {
        if (!branchNum) {
            branchNum = 1;
        }
     ...
     */
    stepToProcedure (thread: any, procedureCode: any, isLambda: any) {
        const definition = isLambda ? procedureCode : thread.target.blocks.getProcedureDefinition(procedureCode);
        if (!definition) {
            return;
        }
        // Check if the call is recursive.
        // If so, set the thread to yield after pushing.
        // dango: lambda never recursive
        const isRecursive = isLambda ? false : thread.isRecursiveCall(procedureCode);
        ...
        if (thread.peekStackFrame().warpMode &&
            ...
            let innerBlock;
            if (!isLambda) {
                innerBlock = thread.target.blocks.getBlock(definitionBlock.inputs.custom_block.block);
            }
            let doWarp = false;
