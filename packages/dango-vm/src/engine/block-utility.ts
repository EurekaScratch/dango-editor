...
import {BlockCached} from './execute';
...
class BlockUtility {
    ...
    get stackFrame () {
        ...
        if (!frame) throw new Error('stackFrame not found in this thread');
     ...
     */
    yield () {
        if (!this.thread) throw new Error('param has not been initialized yet');
     ...
     * @param {string} procedureCode Procedure code for procedure to start, or lambda expression id.
     * @param {boolean} isLambda Whether is a lambda expression.
     */
    startProcedure (procedureCode: string, isLambda: boolean) {
        this.sequencer?.stepToProcedure(this.thread, procedureCode, isLambda);
    }
    /**
     ...
     */
    pushThreadStack (blockId: string) {
    ...
    reuseStackForNextBlock (blockId: string) {
        this.thread?.reuseStackForNextBlock(blockId);
    }
    /**
    ...
    getProcedureParamNamesAndIds (procedureCode: string) {
        if (!this.thread?.target) return null;
     ...
     */
    startHats (requestedHat: string, optMatchFields: Record<string, unknown>, optTarget?: Target) {
     ...
     */
    ioQuery (device: string, func: string, args: unknown[]) {
        // Find the I/O device and execute the query/function call.
        if (this.sequencer?.runtime.ioDevices[device] &&
