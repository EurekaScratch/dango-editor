...
import ExtensionManager, {
    ...
    CategoryInfo,
    Peripheral
} from '../extension-support/extension-manager';
import Blocks from './blocks';
import * as BlocksRuntimeCache from './blocks-runtime-cache';
...
import scratch3Looks from '../blocks/scratch3_looks';
import scratch3Motion from '../blocks/scratch3_motion';
import scratch3Operators from '../blocks/scratch3_operators';
import scratch3Sound from '../blocks/scratch3_sound';
...
export interface AddonOptions {
    ...
    callback: (args: unknown, util: BlockUtility) => unknown | Promise<unknown>;
    arguments: string[];
    hidden?: boolean;
}
...
export interface ExternalCommunicationMethods {
...
const ArgumentTypeMap = (() => {
    ...
    return map as Record<ArgumentType, ArgumentInfo>;
})();

interface CloudDataManager {
 ...
 * cloud variable.
 */
const cloudDataManager = (cloudOptions: CloudOptions) : CloudDataManager => {
    let count = 0;
...
class Runtime extends EventEmitter {
     ...
     */
    _cloneCounter = 0;
    _defaultStoredSettings: ProjectOptions;
    /**
    ...
    _lastStepTime: number = Date.now();
    _linkSocketFactory: any;
    _monitorState: any;
    _nonMonitorThreadCount: any;
    _prevMonitorState: any;
    /**
     * Map to look up a block primitive's implementation function by its opcode.
     * This is a two-step lookup: package name first, then primitive name.
     * @type {Object.<string, Function>}
     */
    _primitives: {[packageName: string]: Function};
    /**
    ...
    _scriptGlowsPreviousFrame: any;
    _stageTarget: RenderedTarget | null = null;
    /**
    ...
    addonBlocks: any;
    audioEngine: any;
    canAddCloudVariable: any;
    cloudOptions: CloudOptions;
    compilerOptions: CompilerOptions  = {
        enabled: false,
        warpTimer: false
    };
    currentMSecs: number | null = null;
    /**
    ...
    externalCommunicationMethods: ExternalCommunicationMethods  = {
    ...
    /** @type {!Sequencer} */
    sequencer: Sequencer;
    stageHeight: number = Runtime.STAGE_HEIGHT;
    stageWidth: number = Runtime.STAGE_WIDTH;
    storage: any;
    /**
    ...
    threadMap: Map<string, Thread> = new Map();
    /**
    ...
    _initializeRemoveCloudVariable (newCloudDataManager: any) {
        return (() => {
            const hadCloudVarsBefore = this.hasCloudData();
    ...
    compilerRegisterExtension (name: string, extensionObject: any) {
        // @ts-expect-error TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
     ...
     * @private
     */
    private _makeExtensionMenuId (menuName: string, extensionId: string) {
     ...
     */
    makeMessageContextForTarget (target?: RenderedTarget) {
        const context = {};
        target = target || this.getEditingTarget() || this.getTargetForStage();
        if (target) {
            // @ts-expect-error TS(2339): Property 'targetType' does not exist on type '{}'.
    ...
    _registerExtensionPrimitives (extensionInfo: ExtensionMetadata) {
        ...
        if (extensionInfo.color1) {
            // @ts-expect-error TS(2339): Property 'color1' does not exist on type '{ id: an... Remove this comment to see the full error message
            categoryInfo.color1 = extensionInfo.color1;
            // @ts-expect-error TS(2339): Property 'color2' does not exist on type '{ id: an... Remove this comment to see the full error message
            categoryInfo.color2 = extensionInfo.color2;
            // @ts-expect-error TS(2339): Property 'color3' does not exist on type '{ id: an... Remove this comment to see the full error message
        ...
        // @ts-expect-error TS(2339): Property 'customFieldTypes' does not exist on type... Remove this comment to see the full error message
    ...
    _refreshExtensionPrimitives (extensionInfo: ExtensionMetadata) {
        const categoryInfo = this._blockInfo.find((info: any) => info.id === extensionInfo.id);
        if (categoryInfo) {
            // @ts-expect-error TS(2554): Expected 3 arguments, but got 1.
     ...
     * @private
     */
    private _fillExtensionCategory (categoryInfo: CategoryInfo, extensionInfo: ExtensionMetadata) {
        categoryInfo.blocks = [];
        ...
        for (const blockInfo of extensionInfo.blocks) {
            try {
                ...
                if (!convertedBlock) continue;
                categoryInfo.blocks.push(convertedBlock);
                if (convertedBlock.json) {
                    // @ts-expect-error TS(2339): Property 'json' does not exist on type '{ info: an... Remove this comment to see the full error message
                    const opcode = convertedBlock.json.type;
                    if ((blockInfo as ExtensionBlockMetadata).blockType !== BlockType.EVENT) {
                        this._primitives[opcode] = convertedBlock.info.func;
                    }
                    if ((blockInfo as ExtensionBlockMetadata).blockType === BlockType.EVENT ||
                        (blockInfo as ExtensionBlockMetadata).blockType === BlockType.HAT) {
                        this._hats[opcode] = {
                            edgeActivated: (blockInfo as ExtensionBlockMetadata).isEdgeActivated,
                            restartExistingThreads: (blockInfo as ExtensionBlockMetadata).shouldRestartExistingThreads
                        };
                    }
                }
            } catch (e) {
     ...
     * @private
     */
    private _convertMenuItems (menuItems: MenuItems) {
     ...
     * @private
     */
    _buildMenuForScratchBlocks (menuName: string, menuInfo: ExtensionMenu, categoryInfo: CategoryInfo) {
    ...
    _buildCustomFieldInfo (fieldName: any, fieldInfo: any, extensionId: string, categoryInfo: CategoryInfo) {
     ...
     */
    _buildCustomFieldTypeForScratchBlocks (fieldName: any, output: any, outputShape: any, categoryInfo: any) {
        return {
            json: {
                type: fieldName,
                message0: '%1',
     ...
     * @private
     */
    _convertForScratchBlocks (blockInfo: ExtensionBlockMetadata | string, categoryInfo: CategoryInfo) : ConvertedBlockInfo {
        if (typeof blockInfo === 'string') {
            if (blockInfo === '---') {
                return this._convertSeparatorForScratchBlocks(blockInfo);
            }
            throw new Error('illegal string blockInfo: ' + blockInfo);
        }
     ...
     * @private
     */
    _convertBlockForScratchBlocks (blockInfo: ExtensionBlockMetadata, categoryInfo: CategoryInfo) : ConvertedBlockInfo {
        ...
        const context: ScratchBlocksContext = {
        ...
        if (iconURI) {
            // @ts-expect-error TS(2339): Property 'extensions' does not exist on type '{ ty... Remove this comment to see the full error message
            blockJSON.extensions = ['scratch_extension'];
            // @ts-expect-error TS(2339): Property 'message0' does not exist on type '{ type... Remove this comment to see the full error message
            ...
            // @ts-expect-error TS(2339): Property 'args0' does not exist on type '{ type: s... Remove this comment to see the full error message
            blockJSON.args0 = [
                iconJSON,
                separatorJSON
            ];
        ...
        case BlockType.COMMAND:
            // @ts-expect-error TS(2339): Property 'outputShape' does not exist on type '{ t... Remove this comment to see the full error message
            blockJSON.outputShape = ScratchBlocksConstants.OUTPUT_SHAPE_SQUARE;
            // @ts-expect-error TS(2339): Property 'previousStatement' does not exist on typ... Remove this comment to see the full error message
            blockJSON.previousStatement = null; // null = available connection; undefined = hat
            if (!blockInfo.isTerminal) {
                // @ts-expect-error TS(2339): Property 'nextStatement' does not exist on type '{... Remove this comment to see the full error message
                blockJSON.nextStatement = null; // null = available connection; undefined = terminal
            }
            break;
        case BlockType.REPORTER:
            // @ts-expect-error TS(2339): Property 'output' does not exist on type '{ type: ... Remove this comment to see the full error message
        ...
        while (inTextNum < blockText.length || (blockInfo.branchCount && inBranchNum < blockInfo.branchCount)) {
            ...
            if (blockInfo.branchCount && inBranchNum < blockInfo.branchCount) {
                // @ts-expect-error TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
        ...
        if (blockInfo.blockType === BlockType.REPORTER) {
            if (!blockInfo.disableMonitor && context.inputList.length === 0) {
                // @ts-expect-error TS(2339): Property 'checkboxInFlyout' does not exist on type... Remove this comment to see the full error message
     ...
     * @param {string} blockInfo - the block to convert
     * @param {CategoryInfo} categoryInfo - the category for this block
     * @returns {ConvertedBlockInfo} - the converted & original block information
     * @private
     */
    _convertSeparatorForScratchBlocks (blockInfo: string) : ConvertedBlockInfo {
        return {
            info: blockInfo,
            xml: '<sep gap="36"/>'
        };
     ...
     * @private
     */
    _convertButtonForScratchBlocks (buttonInfo: ExtensionBlockMetadata) : ConvertedBlockInfo {
        ...
        if (buttonInfo.func && supportedCallbackKeys.indexOf(buttonInfo.func) < 0) {
     ...
     * @private
     */
    _convertPlaceholders (context: ScratchBlocksContext, match: string, placeholder: string) {
        // Sanitize the placeholder to ensure valid XML
        placeholder = placeholder.replace(/[<"&]/, '_');
        if (!context.blockInfo.arguments) throw new Error('arguments not found in context');
        // Determine whether the argument type is one of the known standard field types
        const argInfo = context.blockInfo.arguments[placeholder];
        if (!argInfo) throw new Error('argument is undefined');
        ...
        if (!ArgumentTypeMap[argInfo.type] && context.categoryInfo.customFieldTypes && context.categoryInfo.customFieldTypes[argInfo.type]) {
        ...
        if (argTypeInfo.fieldType === 'field_image') {
            argJSON = this._constructInlineImageJson(argInfo as ExtensionArgumentMetadata);
        } else {
            ...
            const defaultValue = typeof (argInfo as ExtensionArgumentMetadata).defaultValue === 'undefined' ? '' :
                // @ts-expect-error TS(2554): Expected 3 arguments, but got 2.
            ...
            if (argTypeInfo.check) {
                ...
                // @ts-expect-error TS(2339): Property 'check' does not exist on type '{ type: s... Remove this comment to see the full error message
                argJSON.check = argTypeInfo.check;
            }
            let valueName;
            let shadowType;
            let fieldName;
            if (argInfo.hasOwnProperty('menu')) {
                const menuIndex = (argInfo as ExtensionArgumentMetadata).menu;
                const menuInfo = (context.categoryInfo.menuInfo ?? {})[menuIndex as string];
                if (menuInfo?.acceptReporters) {
                    valueName = placeholder;
                    shadowType = this._makeExtensionMenuId(menuIndex as string, context.categoryInfo.id);
                    fieldName = (argInfo as ExtensionArgumentMetadata).menu;
                } else {
                    argJSON.type = 'field_dropdown';
                    // @ts-expect-error TS(2339): Property 'options' does not exist on type '{ type:... Remove this comment to see the full error message
    ...
    getBlocksXML (target?: Target) {
        return this._blockInfo.map((categoryInfo: any) => {
            ...
            const paletteBlocks = categoryInfo.blocks.filter((block: any) => {
            ...
            return {
                id: categoryInfo.id,
                xml: `<category name="${name}" id="${categoryInfo.id}" ${statusButtonXML} ${colorXML} ${menuIconXML}>${paletteBlocks.map((block: any) => block.xml).join('')}</category>`
            };
        });
    }
    /**
     ...
     */
    getBlocksJSON () {
        return this._blockInfo.reduce((result: any, categoryInfo: any) => result.concat(categoryInfo.blocks.map((blockInfo: any) => blockInfo.json)), []);
    }
    /**
     ...
     */
    getScratchLinkSocket (type: string) {
     ...
     */
    _defaultScratchLinkSocketFactory (type: string) {
     ...
     */
    registerPeripheralExtension (extensionId: string, extension: any) {
     ...
     */
    attachV2BitmapAdapter (bitmapAdapter: ScratchSVGRenderer.BitmapAdapter) {
    ...
    attachStorage (storage: ScratchStorage) {
        ...
        if (this.isPackaged) {
            ...
            storage.createAsset = function packagedCreateAsset (assetType: any, dataFormat: any, data: any, assetId: any, generateId: any) {
                if (!assetId) {
     ...
     */
    _pushThread (id: string, target: RenderedTarget, opts: any) {
    ...
    emitCompileError (target: Target, error: unknown) {
     ...
     */
    toggleScript (topBlockId: string, opts?: {target?: RenderedTarget, stackClick?: boolean}) {
        opts = Object.assign({
            target: this._editingTarget,
            stackClick: false
        }, opts);
        if (!opts.target) throw new Error('no target available');
     ...
     */
    addMonitorScript (topBlockId: string, optTarget: RenderedTarget) {
        if (!optTarget) {
            if (this._editingTarget) optTarget = this._editingTarget;
            else throw new Error('no target available');
        }
        for (let i = 0; i < this.threads.length; i++) {
     ...
     */
    allScriptsDo (f: (script: any, target: RenderedTarget) => void, optTarget?: RenderedTarget) {
    ...
    allScriptsByOpcodeDo (opcode: string, f: (script: any, target: RenderedTarget) => void, optTarget: any) {
        let targets = this.executableTargets;
        if (optTarget) {
            targets = [optTarget];
        }
     ...
     */
    startHats (requestedHatOpcode: string, optMatchFields: any, optTarget: any) {
        ...
        for (const opts in optMatchFields) {
            if (!optMatchFields.hasOwnProperty(opts)) {
                continue;
            }
        ...
        this.allScriptsByOpcodeDo(requestedHatOpcode, (script: any, target: any) => {
        ...
        // @ts-expect-error TS(7006): Parameter 'thread' implicitly has an 'any' type.
    ...
    disposeTarget (disposingTarget: RenderedTarget) {
        this.targets = this.targets.filter((target: any) => {
    ...
    greenFlag () {
        ...
        this.updateCurrentMSecs();
        this.ioDevices.clock.resetProjectTimer();
        this.targets.forEach((target: any) => target.clearEdgeActivatedValues());
    ...
    _step () {
        ...
        this.threads = this.threads.filter((thread: any) => !thread.isKilled);
        this.updateThreadMap();
        // Find all edge-activated hats, and add them to threads to be evaluated.
        for (const hatType in this._hats) {
            if (!this._hats.hasOwnProperty(hatType)) {
                continue;
            }
        ...
        this._emitProjectRunStatus(this.threads.length + doneThreads.length -
     ...
     */
    setInterpolation (interpolationEnabled: boolean) {
     ...
     */
    setRuntimeOptions (runtimeOptions: object) {
     ...
     */
    setCompilerOptions (compilerOptions: object) {
     ...
     * @param {boolean} [hidden] True to not include this block in the block palette
     */
    addAddonBlock (options: AddonOptions) {
        const procedureCode = options.procedureCode;
        const names = options.arguments;
        const ids = options.arguments.map((_: any, i: any) => `arg${i}`);
        ...
        if (!options.hidden) {
            const ID = 'a-b';
            let blockInfo = this._blockInfo.find((i: any) => i.id === ID);
            if (!blockInfo) {
    ...
    getAddonBlock (procedureCode: string) {
    ...
    parseProjectOptions () {
        ...
        // @ts-expect-error TS(2571): Object is of type 'unknown'.
        const lineWithMagic = comment.text.split('\n').find((i: any) => i.endsWith(COMMENT_CONFIG_MAGIC));
        if (!lineWithMagic) {
        ...
        // @ts-expect-error TS(2339): Property 'framerate' does not exist on type 'objec... Remove this comment to see the full error message
        if (typeof parsed.framerate === 'number') {
            // @ts-expect-error TS(2339): Property 'framerate' does not exist on type 'objec... Remove this comment to see the full error message
            this.setFramerate(parsed.framerate);
        }
        // @ts-expect-error TS(2339): Property 'turbo' does not exist on type 'object'.
        if (parsed.turbo) {
            this.turboMode = true;
            this.emit(Runtime.TURBO_MODE_ON);
        }
        // @ts-expect-error TS(2339): Property 'interpolation' does not exist on type 'o... Remove this comment to see the full error message
        if (parsed.interpolation) {
            this.setInterpolation(true);
        }
        // @ts-expect-error TS(2339): Property 'runtimeOptions' does not exist on type '... Remove this comment to see the full error message
        if (parsed.runtimeOptions) {
            // @ts-expect-error TS(2339): Property 'runtimeOptions' does not exist on type '... Remove this comment to see the full error message
            this.setRuntimeOptions(parsed.runtimeOptions);
        }
        // @ts-expect-error TS(2339): Property 'hq' does not exist on type 'object'.
        if (parsed.hq && this.renderer) {
            this.renderer.setUseHighQualityRender(true);
        }
        // @ts-expect-error TS(2339): Property 'width' does not exist on type 'object'.
        const storedWidth = +parsed.width || this.stageWidth;
        // @ts-expect-error TS(2339): Property 'height' does not exist on type 'object'.
     ...
     */
    precompile () {
        this.allScriptsDo((topBlockId: string, target: RenderedTarget) => {
     ...
     */
    emitBlockEndDrag (blocks: any, topBlockId: string) {
     ...
     */
    requestUpdateMonitor (monitor: Map<string, unknown>) {
        const id = monitor.get('id');
        if (this._monitorState.has(id)) {
            this._monitorState =
                // Use mergeWith here to prevent undefined values from overwriting existing ones
                this._monitorState.set(id, this._monitorState.get(id).mergeWith((prev: any, next: any) => {
    ...
    requestHideMonitor (monitorId: string) {
        return this.requestUpdateMonitor(new Map<string, unknown>([
            ['id', monitorId],
            ['visible', false]
        ]));
    }
    ...
    requestRemoveMonitorByTargetId (targetId: string) {
        this._monitorState = this._monitorState.filterNot((value: any) => value.targetId === targetId);
    }
    /**
    ...
    getAllVarNamesOfType (varType: any) {
        let varNames: any = [];
        for (const target of this.targets) {
    ...
    getLabelForOpcode (extendedOpcode: string) {
        ...
        const categoryInfo = this._blockInfo.find((ci: any) => ci.id === category);
        if (!categoryInfo) {
            return;
        }
        if (!categoryInfo.blocks) return;
        const block = categoryInfo.blocks.find((b: any) => b.info.opcode === opcode);
        if (!block) {
            return;
        }
    ...
    createNewGlobalVariable (variableName: string, optVarId: string, optVarType: string) {
        ...
        if (!stage) throw new Error('stage target is undefined');
    ...
    updatePrivacy () {
        const enforceRestrictions = (this.enforcePrivacy &&
     ...
     */
    setEnforcePrivacy (enabled: boolean) {
     ...
     */
    setExternalCommunicationMethod (method: string, enabled: boolean) {
        if (!Object.prototype.hasOwnProperty.call(this.externalCommunicationMethods, method)) {
            throw new Error(`Unknown method: ${method}`);
        }
        this.externalCommunicationMethods[method as keyof typeof this.externalCommunicationMethods] = enabled;
        this.updatePrivacy();
    }
}
