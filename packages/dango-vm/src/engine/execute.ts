...
import * as BlocksExecuteCache from './blocks-execute-cache';
...
// metrics (dd)
const handleReport = function (resolvedValue: any, sequencer: any, thread: any, blockCached: any, lastOperation: any) {
...
const handlePromise = (primitiveReportedValue: any, sequencer: any, thread: any, blockCached: any, lastOperation: any) => {
    if (thread.status === Thread.STATUS_RUNNING) {
        // Primitive returned a promise; automatically yield thread.
        thread.status = Thread.STATUS_PROMISE_WAIT;
    }
    // Promise handlers
    primitiveReportedValue.then((resolvedValue: any) => {
...
export class BlockCached {
    ...
    mutation: any;
    opcode: any;
    constructor (blockContainer: Blocks, cached: any) {
        if (!blockUtility.sequencer) return;
        /**
        ...
        this._isShadowBlock = (!this._definedBlockFunction &&
            fieldKeys.length === 1 &&
 ...
 */
const _prepareBlockProfiling = function (profiler: any, blockCached: any) {
 ...
 */
const execute = function (sequencer: any, thread: any) {
    ...
    if (currentStackFrame.reported !== null) {
        ...
        for (; i < reported.length; i++) {
            ...
            const opCached = ops.find((op: any) => op.id === oldOpCached);
            if (opCached) {
        ...
        if (reported.length > 0) {
            const lastExisting = reported.reverse().find((report: any) => ops.find((op: any) => op.id === report.opCached));
            if (lastExisting) {
                i = ops.findIndex((opCached: any) => opCached.id === lastExisting.opCached) + 1;
            } else {
                i = 0;
            }
        }
    ...
    for (; i < length; i++) {
        ...
        if (isPromise(primitiveReportedValue)) {
            ...
            currentStackFrame.reported = ops.slice(0, i).map((reportedCached: any) => {
