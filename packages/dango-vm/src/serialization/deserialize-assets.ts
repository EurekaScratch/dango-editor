 ...
 * occurred.
 */
const deserializeSound = function (sound: any, runtime: any, zip: any, assetFileName: any) {
    ...
    return soundFile.async('uint8array').then((data: any) => storage.createAsset(storage.AssetType.Sound, dataFormat, data, null, true))
        .then((asset: any) => {
            sound.asset = asset;
 ...
 * occurred.
 */
const deserializeCostume = function (costume: any, runtime: any, zip: any, assetFileName: any, textLayerFileName: any) {
    const storage = runtime.storage;
    ...
    if (costume.textLayerMD5) {
        ...
        textLayerFilePromise = textLayerFile.async('uint8array')
            .then((data: any) => storage.createAsset(storage.AssetType.ImageBitmap, 'png', data, costume.textLayerMD5))
            .then((asset: any) => {
    ...
    return Promise.all([textLayerFilePromise,
        costumeFile.async('uint8array')
            .then((data: any) => storage.createAsset(assetType,
