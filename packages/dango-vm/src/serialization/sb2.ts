...
import ScratchXUtilities from '../extension-support/tw-scratchx-utilities';
 ...
 * @returns {boolean}
 */
const isPossiblyScratchXBlock = (opcode: any) => SCRATCHX_OPCODE_SEPARATOR.test(opcode);
/**
 * @param {string} opcode
 * @returns {string}
 */
const mapScratchXOpcode = (opcode: any) => {
 ...
 * @returns {object}
 */
const mapScratchXBlock = (block: any) => {
 ...
 */
const parseProcedureArgMap = function (procCode: any) {
    const argMap = [
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        if (part.substring(0, 1) === '%') {
            ...
            if (argType === 'n') {
                // @ts-expect-error TS(2339): Property 'inputOp' does not exist on type '{ type:... Remove this comment to see the full error message
 ...
 */
const parseProcedureArgIds = function (procCode: any) {
    return parseProcedureArgMap(procCode)
        // @ts-expect-error TS(2339): Property 'inputName' does not exist on type '{}'.
 ...
 */
// @ts-expect-error TS(7023): 'flatten' implicitly has return type 'any' because... Remove this comment to see the full error message
const flatten = function (blocks: any) {
    let finalBlocks = [];
 ...
 */
const parseBlockList = function (blockList: any, addBroadcastMsg: any, getVariableId: any, extensions: any, parseState: any, comments: any, commentIndex: any) {
    const resultingList = [];
 ...
 */
const parseScripts = function (scripts: any, blocks: any, addBroadcastMsg: any, getVariableId: any, extensions: any, comments: any) {
...
const generateVariableIdGetter = (function () {
    ...
    const namer = (targetId: any, name: any, type: any) => `${targetId}-${StringUtil.replaceUnsafeChars(name)}-${type}`;
    return function (targetId: any, topLevel: any) {
        // Reset the global variable map if topLevel
        if (topLevel) {
            globalVariableNameMap = {};
        }
        return function (name: any, type: any) {
...
const globalBroadcastMsgStateGenerator = (function () {
    ...
    const allBroadcastFields: any = [];
    const emptyStringName = uid();
    return function (topLevel: any) {
        if (topLevel) {
            broadcastMsgNameMap = {};
        }
        return {
            broadcastMsgMapUpdater: function (name: any, field: any) {
 ...
 * @param {ImportedExtensionsInfo} extensions - (in/out) parsed extension information will be stored here.
 */
const parseMonitorObject = (object: any, runtime: any, targets: any, extensions: any) => {
    ...
    target = target || targets.filter((t: any) => t.getName() === object.target)[0];
    if (!target) {
 ...
 *   objects.
 */
const parseScratchAssets = function (object: any, runtime: any, topLevel: any, zip: any) {
    ...
    if (object.hasOwnProperty('costumes')) {
        for (let i = 0; i < object.costumes.length; i++) {
            ...
            // @ts-expect-error TS(2339): Property 'dataFormat' does not exist on type '{ na... Remove this comment to see the full error message
            costume.dataFormat = ext;
            // @ts-expect-error TS(2339): Property 'assetId' does not exist on type '{ name:... Remove this comment to see the full error message
            costume.assetId = md5;
            if (costumeSource.textLayerMD5) {
                // @ts-expect-error TS(2339): Property 'textLayerMD5' does not exist on type '{ ... Remove this comment to see the full error message
            ...
            // @ts-expect-error TS(2345): Argument of type 'Promise<any>' is not assignable ... Remove this comment to see the full error message
    ...
    if (object.children) {
        for (let m = 0; m < object.children.length; m++) {
            // @ts-expect-error TS(2345): Argument of type '{ costumePromises: never[]; soun... Remove this comment to see the full error message
 ...
 */
const parseScratchObject = function (object: any, runtime: any, extensions: any, topLevel: any, zip: any, assets: any) {
    if (!object.hasOwnProperty('objName')) {
    ...
    if (object.hasOwnProperty('scriptComments')) {
        const comments = object.scriptComments.map((commentDesc: any) => {
    ...
    for (const commentIndex in blockComments) {
        ...
        currBlockComments.forEach((c: any) => {
...
const reorderParsedTargets = function (targets: any) {
    // Reorder parsed targets based on the temporary targetPaneOrder property
    // and then delete it.
    const reordered = targets.map((t: any, index: any) => {
        t.layerOrder = index;
        return t;
    }).sort((a: any, b: any) => a.targetPaneOrder - b.targetPaneOrder);
    // Delete the temporary target pane ordering since we shouldn't need it anymore.
    reordered.forEach((t: any) => {
 ...
 */
const sb2import = function (json: any, runtime: any, optForceSprite: any, zip: any) {
    const extensions = {
 ...
 */
const specMapBlock = function (block: any) {
    const opcode = block[0];
    // @ts-expect-error TS(7053): Element implicitly has an 'any' type because expre... Remove this comment to see the full error message
 ...
 */
const parseBlock = function (sb2block: any, addBroadcastMsg: any, getVariableId: any, extensions: any, parseState: any, comments: any, commentIndex: any) {
    ...
    if (!blockMetadata) {
        ...
        if (commentsForParsedBlock) {
            commentsForParsedBlock.forEach((comment: any) => {
    ...
    if (commentsForParsedBlock) {
        ...
        // @ts-expect-error TS(2339): Property 'comment' does not exist on type '{ id: s... Remove this comment to see the full error message
    ...
    switch (oldOpcode) {
    case 'comeToFront':
        // @ts-expect-error TS(2339): Property 'FRONT_BACK' does not exist on type '{}'.
        ...
        break;
    case 'goBackByLayers:':
        // @ts-expect-error TS(2339): Property 'FORWARD_BACKWARD' does not exist on type... Remove this comment to see the full error message
    ...
    case 'backgroundIndex':
        // @ts-expect-error TS(2339): Property 'NUMBER_NAME' does not exist on type '{}'... Remove this comment to see the full error message
