...
import compress from './tw-compress-sb3';
 ...
 */
const serializePrimitiveBlock = function (block: any) {
 ...
 */
const serializeInputs = function (inputs: any) {
 ...
 */
const serializeFields = function (fields: any) {
    const obj = Object.create(null);
 ...
 * if not.
 */
const serializeBlock = function (block: any) {
 ...
 */
const compressInputTree = function (block: any, blocks: any) {
 ...
 */
const getExtensionIdForOpcode = function (opcode: string) {
 ...
 */
const serializeBlocks = function (blocks: any) {
    const obj = Object.create(null);
 ...
 */
const serializeCostume = function (costume: any) {
    const obj = Object.create(null);
 ...
 */
const serializeSound = function (sound: any) {
    const obj = Object.create(null);
    obj.name = sound.name;
...
const isVariableValueSafeForJSON = (value: unknown) => typeof value === 'number' ||
    typeof value === 'string' ||
    typeof value === 'boolean';
const makeSafeForJSON = (value: any) => {
 ...
 */
const serializeVariables = function (variables: any) {
    const obj = Object.create(null);
...
const serializeComments = function (comments: any) {
    const obj = Object.create(null);
 ...
 */
const serializeTarget = function (target: any, extensions: any) {
    const obj = Object.create(null);
    ...
    if (target.hasOwnProperty('volume')) {
    ...
    if (obj.isStage) { // Only the stage should have these properties
        if (target.hasOwnProperty('tempo')) {
    ...
    targetExtensions.forEach((extensionId: any) => {
        extensions.add(extensionId);
    });
    return obj;
};
const getSimplifiedLayerOrdering = function (targets: any) {
    const layerOrders = targets.map((t: any) => t.getLayerOrder());
    return MathUtil.reducedSortOrdering(layerOrders);
};
const serializeMonitors = function (monitors: any, runtime: any) {
    ...
    return monitors.valueSeq()
        ...
        .map((monitorData: any) => {
            ...
            if (monitorData.mode !== 'list') {
                // @ts-expect-error TS(2339): Property 'sliderMin' does not exist on type '{ id:... Remove this comment to see the full error message
                serializedMonitor.sliderMin = monitorData.sliderMin;
                // @ts-expect-error TS(2339): Property 'sliderMax' does not exist on type '{ id:... Remove this comment to see the full error message
                serializedMonitor.sliderMax = monitorData.sliderMax;
                // @ts-expect-error TS(2339): Property 'isDiscrete' does not exist on type '{ id... Remove this comment to see the full error message
 ...
 */
const serialize = function (runtime: Runtime, targetId?: string, {allowOptimization = true} = {}) {
    // Fetch targets
    const obj = Object.create(null);
    ...
    const originalTargetsToSerialize = targetId ?
        ...
        runtime.targets.filter((target: any) => target.isOriginal);
    const layerOrdering = getSimplifiedLayerOrdering(originalTargetsToSerialize);
    const flattenedOriginalTargets = originalTargetsToSerialize.map((t: any) => t.toJSON());
    ...
    if (runtime.renderer && !targetId) {
        flattenedOriginalTargets.forEach((t: any, index: any) => {
            t.layerOrder = layerOrdering[index];
        });
    }
    const serializedTargets = flattenedOriginalTargets.map((t: any) => serializeTarget(t, extensions));
    if (targetId) {
 ...
 */
const deserializeInputDesc = function (inputDescOrId: any, parentId: any, isShadow: any, blocks: any) {
 ...
 */
const deserializeInputs = function (inputs: any, parentId: any, blocks: any) {
 ...
 */
const deserializeFields = function (fields: any) {
    // Explicitly not using Object.create(null) here
    // because we call prototype functions later in the vm
    const obj = {};
    for (const fieldName in fields) {
 ...
 */
const deserializeBlocks = function (blocks: any) {
    for (const blockId in blocks) {
 ...
 */
const parseScratchAssets = function (object: any, runtime: Runtime, zip: JSZip) {
    ...
    // Costumes from JSON.
    assets.costumePromises = (object.costumes || []).map((costumeSource: any) => {
        ...
        // @ts-expect-error TS(2339): Property 'md5' does not exist on type '{ asset: an... Remove this comment to see the full error message
        costume.md5 = costumeMd5Ext;
        // @ts-expect-error TS(2339): Property 'dataFormat' does not exist on type '{ as... Remove this comment to see the full error message
    ...
    // Sounds from JSON
    assets.soundPromises = (object.sounds || []).map((soundSource: any) => {
        const sound = {
 ...
 */
const parseScratchObject = function (object: any, runtime: Runtime, extensions: ImportedExtensionsInfo, zip: JSZip, assets: any) {
    if (!object.hasOwnProperty('name')) {
        // Watcher/monitor - skip this object until those are implemented in VM.
        // @todo
        return Promise.resolve(null);
    ...
    if (object.hasOwnProperty('blocks')) {
        ...
        for (const blockId in object.blocks) {
            if (!object.blocks.hasOwnProperty(blockId)) {
                continue;
            }
    ...
    if (object.hasOwnProperty('targetPaneOrder')) {
        ...
        // @ts-expect-error TS(2339): Property 'targetPaneOrder' does not exist on type ... Remove this comment to see the full error message
...
const deserializeMonitor = function (monitorData: any, runtime: Runtime, targets: any, extensions: ImportedExtensionsInfo) {
    // Monitors position is always stored as position from top-left corner in 480x360 stage.
    const xOffset = (runtime.stageWidth - 480) / 2;
    const yOffset = (runtime.stageHeight - 360) / 2;
    ...
    if (monitorData.spriteName) {
        const filteredTargets = targets.filter((t: any) => t.sprite.name === monitorData.spriteName);
    ...
    if (monitorData.opcode === 'data_listcontents') {
        const listTarget = monitorData.targetId ?
            targets.find((t: any) => t.id === monitorData.targetId) :
            targets.find((t: any) => t.isStage);
        if (listTarget &&
    ...
    if (existingMonitorBlock) {
        ...
        if (monitorData.opcode === 'data_variable') {
            // @ts-expect-error TS(2339): Property 'VARIABLE' does not exist on type '{}'.
            ...
            // @ts-expect-error TS(2339): Property 'LIST' does not exist on type '{}'.
...
const replaceUnsafeCharsInVariableIds = function (targets: Target[]) {
 ...
 */
const deserialize = function (json: any, runtime: Runtime, zip: JSZip, isSingleSprite: boolean) : Promise<ImportedProject> {
    const extensions = {
        extensionIDs: new Set<string>(),
        extensionURLs: new Map<string, string>()
    };
    ...
    const targetObjects = ((isSingleSprite ? [json] : json.targets) || [])
        .map((t: any, i: any) => Object.assign(t, {targetPaneOrder: i}))
        .sort((a: any, b: any) => a.layerOrder - b.layerOrder);
    const monitorObjects = json.monitors || [];
    return Promise.resolve(targetObjects.map((target: any) => parseScratchAssets(target, runtime, zip)))
        // Force this promise to wait for the next loop in the js tick. Let
        // storage have some time to send off asset requests.
        .then(assets => Promise.resolve(assets))
        .then(assets => Promise.all(targetObjects
            .map((target: any, index: any) => parseScratchObject(target, runtime, extensions, zip, assets[index]))))
        ...
        .then(targets => {
            monitorObjects.map((monitorDesc: any) => deserializeMonitor(monitorDesc, runtime, targets, extensions));
            return targets;
        })
        .then(targets => ({
            targets,
