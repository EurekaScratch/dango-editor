class MathUtil {
     ...
     */
    static wrapClamp (n: number, min: number, max: number) {
    ...
    static reducedSortOrdering (elts: number[]) {
        const sorted = elts.slice(0).sort((a: any, b: any) => a - b);
     ...
     */
    static inclusiveRandIntWithout (lower: number, upper: number, excluded: number) {
     ...
     */
    static scale (i: number, iMin: number, iMax: number, oMin: number, oMax: number) {
