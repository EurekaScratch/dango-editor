...
class TaskQueue {
     ...
     * @memberof TaskQueue
     */
    constructor (maxTokens: any, refillRate: any, options = {}) {
        this._maxTokens = maxTokens;
        this._refillRate = refillRate;
        this._pendingTaskRecords = [];
        // @ts-expect-error TS(2339): Property 'startingTokens' does not exist on type '... Remove this comment to see the full error message
        this._tokenCount = options.hasOwnProperty('startingTokens') ? options.startingTokens : maxTokens;
        // @ts-expect-error TS(2339): Property 'maxTotalCost' does not exist on type '{}... Remove this comment to see the full error message
    ...
    do (task: any, cost = 1) {
        if (this._maxTotalCost < Infinity) {
            const currentTotalCost = this._pendingTaskRecords.reduce((t: any, r: any) => t + r.cost, 0);
        ...
        // @ts-expect-error TS(2339): Property 'promise' does not exist on type '{ cost:... Remove this comment to see the full error message
        newRecord.promise = new Promise((resolve, reject) => {
            // @ts-expect-error TS(2339): Property 'cancel' does not exist on type '{ cost: ... Remove this comment to see the full error message
            newRecord.cancel = () => {
                reject(new Error('Task canceled'));
            };

            // The caller, `_runTasks()`, is responsible for cost-checking and spending tokens.
            // @ts-expect-error TS(2339): Property 'wrappedTask' does not exist on type '{ c... Remove this comment to see the full error message
     ...
     * @memberof TaskQueue
     */
    cancel (taskPromise: any) {
        const taskIndex = this._pendingTaskRecords.findIndex((r: any) => r.promise === taskPromise);
        if (taskIndex !== -1) {
    ...
    cancelAll () {
        ...
        this._pendingTaskRecords = [];
        oldTasks.forEach((r: any) => r.cancel());
    }
