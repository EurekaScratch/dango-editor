...
class Scratch3LooksBlocks {
    ...
    _positionBubble (target: any) {
        ...
        if (bubbleState.onSpriteRight && bubbleWidth + targetBounds.right > stageBounds.right &&
            ...
            this.runtime.renderer.updateDrawablePosition(bubbleState.drawableId, [
                bubbleState.onSpriteRight ? (Math.max(stageBounds.left, // Bubble should not extend past left edge of stage
     ...
     * @private
     */
    _updateBubble (target: any, type: any, text: any) {
        const bubbleState = this._getBubbleState(target);
    ...
    getMonitored () {
        return {
            looks_size: {
                ...
                getId: (targetId: any) => `${targetId}_size`
            },
            looks_costumenumbername: {
                isSpriteSpecific: true,
                getId: (targetId: any, fields: any) => getMonitorIdForBlockWithArgs(`${targetId}_costumenumbername`, fields)
            },
            looks_backdropnumbername: {
                getId: (_: any, fields: any) => getMonitorIdForBlockWithArgs('backdropnumbername', fields)
            }
        };
    }
    say (args: any, util: any) {
        // @TODO in 2.0 calling say/think resets the right/left bias of the bubble
        const message = args.MESSAGE;
        this._say(message, util.target);
    }
    _say (message: any, target: any) {
        this.runtime.emit(Scratch3LooksBlocks.SAY_OR_THINK, target, 'say', message);
    }
    sayforsecs (args: any, util: any) {
        this.say(args, util);
    ...
    show (args: any, util: any) {
        util.target.setVisible(true);
        this._renderBubble(util.target);
    }
    hide (args: any, util: any) {
     ...
     */
    _setCostume (target: any, requestedCostume: any, optZeroIndex: any) {
     ...
     * @return {Array.<!Thread>} Any threads started by this switch.
     */
    _setBackdrop (stage: any, requestedBackdrop: any, optZeroIndex: any) {
    ...
    switchBackdropAndWait (args: any, util: any) {
        ...
        const waiting = util.stackFrame.startedThreads
            .some((thread: any) => instance.runtime.threads.indexOf(thread) !== -1);
        if (waiting) {
            // If all threads are waiting for the next tick or later yield
            // for a tick as well. Otherwise yield until the next loop of
            // the threads.
            if (util.stackFrame.startedThreads
                .every((thread: any) => instance.runtime.isWaitingThread(thread))) {
                util.yieldTick();
            } else {
                util.yield();
            }
    ...
    clampEffect (effect: any, value: any) {
    ...
    changeEffect (args: any, util: any) {
    ...
    setEffect (args: any, util: any) {
        const effect = Cast.toString(args.EFFECT).toLowerCase();
        let value = Cast.toNumber(args.VALUE);
        value = this.clampEffect(effect, value);
        util.target.setEffect(effect, value);
    }
    clearEffects (args: any, util: any) {
        util.target.clearEffects();
    }
    changeSize (args: any, util: any) {
        const change = Cast.toNumber(args.CHANGE);
        util.target.setSize(util.target.size + change);
    }
    setSize (args: any, util: any) {
        const size = Cast.toNumber(args.SIZE);
        util.target.setSize(size);
    }
    goToFrontBack (args: any, util: any) {
    ...
    goForwardBackwardLayers (args: any, util: any) {
        if (!util.target.isStage) {
    ...
    getSize (args: any, util: any) {
        return Math.round(util.target.size);
    }
    getBackdropNumberName (args: any) {
        const stage = this.runtime.getTargetForStage();
    ...
    getCostumeNumberName (args: any, util: any) {
        if (args.NUMBER_NAME === 'number') {
