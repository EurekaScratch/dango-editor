...
class Scratch3DataBlocks {
    ...
    changeVariableBy (args: any, util: any) {
        const variable = util.target.lookupOrCreateVariable(args.VARIABLE.id, args.VARIABLE.name);
    ...
    changeMonitorVisibility (id: any, visible: any) {
    ...
    getListContents (args: any, util: any) {
    ...
    addToList (args: any, util: any) {
        const list = util.target.lookupOrCreateList(args.LIST.id, args.LIST.name);
        list.value.push(args.ITEM);
        list._monitorUpToDate = false;
    }
    deleteOfList (args: any, util: any) {
        const list = util.target.lookupOrCreateList(args.LIST.id, args.LIST.name);
        ...
        list.value.splice((index as number) - 1, 1);
        list._monitorUpToDate = false;
    }
    deleteAllOfList (args: any, util: any) {
        const list = util.target.lookupOrCreateList(args.LIST.id, args.LIST.name);
        list.value = [];
        return;
    }
    insertAtList (args: any, util: any) {
    ...
    replaceItemOfList (args: any, util: any) {
        const item = args.ITEM;
        const list = util.target.lookupOrCreateList(args.LIST.id, args.LIST.name);
    ...
    getItemOfList (args: any, util: any) {
        const list = util.target.lookupOrCreateList(args.LIST.id, args.LIST.name);
        const index = Cast.toListIndex(args.INDEX, list.value.length, false);
        if (index === Cast.LIST_INVALID) {
            return '';
    ...
    getItemNumOfList (args: any, util: any) {
        const item = args.ITEM;
        const list = util.target.lookupOrCreateList(args.LIST.id, args.LIST.name);
    ...
    lengthOfList (args: any, util: any) {
        const list = util.target.lookupOrCreateList(args.LIST.id, args.LIST.name);
        return list.value.length;
    }
    listContainsItem (args: any, util: any) {
        const item = args.ITEM;
        const list = util.target.lookupOrCreateList(args.LIST.id, args.LIST.name);
