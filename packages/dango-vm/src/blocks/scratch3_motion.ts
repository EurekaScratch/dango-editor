...
class Scratch3MotionBlocks {
    ...
    getMonitored () {
        return {
            motion_xposition: {
                ...
                getId: (targetId: any) => `${targetId}_xposition`
            },
            motion_yposition: {
                isSpriteSpecific: true,
                getId: (targetId: any) => `${targetId}_yposition`
            },
            motion_direction: {
                isSpriteSpecific: true,
                getId: (targetId: any) => `${targetId}_direction`
            }
        };
    }
    moveSteps (args: any, util: any) {
        const steps = Cast.toNumber(args.STEPS);
        this._moveSteps(steps, util.target);
    }
    _moveSteps (steps: any, target: any) {
        const radians = MathUtil.degToRad(90 - target.direction);
        const dx = steps * Math.cos(radians);
        const dy = steps * Math.sin(radians);
        target.setXY(target.x + dx, target.y + dy);
    }
    goToXY (args: any, util: any) {
        const x = Cast.toNumber(args.X);
        const y = Cast.toNumber(args.Y);
        util.target.setXY(x, y);
    }
    getTargetXY (targetName: any, util: any) {
        let targetX = 0;
        let targetY = 0;
    ...
    goTo (args: any, util: any) {
        const targetXY = this.getTargetXY(args.TO, util);
        if (targetXY) {
            util.target.setXY(targetXY[0], targetXY[1]);
        }
    }
    turnRight (args: any, util: any) {
        const degrees = Cast.toNumber(args.DEGREES);
        util.target.setDirection(util.target.direction + degrees);
    }
    turnLeft (args: any, util: any) {
        const degrees = Cast.toNumber(args.DEGREES);
        util.target.setDirection(util.target.direction - degrees);
    }
    pointInDirection (args: any, util: any) {
        const direction = Cast.toNumber(args.DIRECTION);
        util.target.setDirection(direction);
    }
    pointTowards (args: any, util: any) {
        let targetX = 0;
        let targetY = 0;
    ...
    setRotationStyle (args: any, util: any) {
        util.target.setRotationStyle(args.STYLE);
    }
    changeX (args: any, util: any) {
        const dx = Cast.toNumber(args.DX);
        util.target.setXY(util.target.x + dx, util.target.y);
    }
    setX (args: any, util: any) {
        const x = Cast.toNumber(args.X);
        util.target.setXY(x, util.target.y);
    }
    changeY (args: any, util: any) {
        const dy = Cast.toNumber(args.DY);
        util.target.setXY(util.target.x, util.target.y + dy);
    }
    setY (args: any, util: any) {
        const y = Cast.toNumber(args.Y);
        util.target.setXY(util.target.x, y);
    }
    getX (args: any, util: any) {
        return this.limitPrecision(util.target.x);
    }
    getY (args: any, util: any) {
        return this.limitPrecision(util.target.y);
    }
    getDirection (args: any, util: any) {
        return util.target.direction;
    }
    // This corresponds to snapToInteger in Scratch 2
    limitPrecision (coordinate: any) {
