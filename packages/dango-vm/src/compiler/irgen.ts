...
import compatBlocks from './compat-blocks';
/**
 ...
 * @param {import('../engine/variable')} varObj The Scratch Variable
 ...
 * @returns {string}
 */
const generateProcedureVariant = (code: any, warp: any) => {
    if (warp) {
        return `W${code}`;
    }
    return `Z${code}`;
 ...
 */
const parseProcedureCode = (variant: any) => variant.substring(1);
/**
 * @param {string} variant Variant generated by generateProcedureVariant()
 * @returns {boolean} true if warp enabled
 */
const parseIsWarp = (variant: any) => variant.charAt(0) === 'W';
class ScriptTreeGenerator {
    ...
    setProcedureVariant (procedureVariant: any) {
        ...
        if (!procedureVariant.startsWith('L')) {
        ...
        // Always yield for lambda expression
        this.script.yields = true;
    }
    enableWarp () {
        this.script.isWarp = true;
    }
    getBlockById (blockId: any) {
    ...
    getBlockInfo (fullOpcode: any) {
        ...
        const categoryInfo = this.runtime._blockInfo.find((ci: any) => ci.id === category);
        if (!categoryInfo) {
            return null;
        }
        const blockInfo = categoryInfo.blocks.find((b: any) => b.info.opcode === opcode);
        if (!blockInfo) {
            return null;
        }
        return blockInfo;
     ...
     */
    // @ts-expect-error TS(7023): 'descendInputOfBlock' implicitly has return type '... Remove this comment to see the full error message
    descendInputOfBlock (parentBlock: any, inputName: any) {
    ...
    descendInput (block: any) {
        ...
        case 'operator_join_advanced': {
            const paramIds = JSON.parse(block.mutation.argumentids);
            const paramDefaults = JSON.parse(block.mutation.argumentdefaults);
            if (paramIds === null) {
                return {
                    kind: 'noop'
                };
        ...
        case 'operator_random': {
            // @ts-expect-error TS(7022): 'from' implicitly has type 'any' because it does n... Remove this comment to see the full error message
            const from = this.descendInputOfBlock(block, 'FROM');
            // @ts-expect-error TS(7022): 'to' implicitly has type 'any' because it does not... Remove this comment to see the full error message
     ...
     */
    // @ts-expect-error TS(7023): 'descendStackedBlock' implicitly has return type '... Remove this comment to see the full error message
    descendStackedBlock (block: any) {
        switch (block.opcode) {
     ...
     */
    // @ts-expect-error TS(7023): 'descendSubstack' implicitly has return type 'any'... Remove this comment to see the full error message
    descendSubstack (parentBlock: any, substackName: any) {
     ...
     * @returns {Node[]} List of stacked block nodes.
     */
    // @ts-expect-error TS(7023): 'walkStack' implicitly has return type 'any' becau... Remove this comment to see the full error message
    walkStack (startingBlockId: any) {
        const result = [];
        ...
        while (blockId !== null) {
            ...
            // @ts-expect-error TS(7022): 'node' implicitly has type 'any' because it does n... Remove this comment to see the full error message
     ...
     */
    descendVariable (block: any, fieldName: any, type: any) {
     ...
     * @returns {*} A parsed variable object.
     */
    _descendVariable (id: any, name: any, type: any) {
    ...
    readTopBlockComment (commentId: any) {
...
class IRGenerator {
    ...
    blocks: any;
    compilingProcedures: any;
    procedures: any;
    proceduresToCompile: any;
    thread: any;
    constructor (thread: any) {
        this.thread = thread;
        this.blocks = thread.blockContainer;
    ...
    addProcedureDependencies (dependencies: any) {
        for (const procedureVariant of dependencies) {
            ...
            const procedureCode = parseProcedureCode(procedureVariant);
            const definition = procedureVariant.startsWith('L') ? this.blocks.getBranch(procedureCode) : this.blocks.getProcedureDefinition(procedureCode);
     ...
     */
    generateScriptTree (generator: any, topBlockId: any) {
