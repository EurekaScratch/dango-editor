 ...
 */
const loadSoundFromAsset = function (sound: any, soundAsset: any, runtime: any, soundBank: any) {
    sound.assetId = soundAsset.assetId;
    if (!runtime.audioEngine) {
        log.warn('No audio engine present; cannot load sound asset: ', sound.md5);
        return Promise.resolve(sound);
    }
    return runtime.audioEngine.decodeSoundPlayer(Object.assign({}, sound, {data: soundAsset.data})).then((soundPlayer: any) => {
...
// in a `broken` field
const handleSoundLoadError = function (sound: any, runtime: any, soundBank: any) {
    ...
    return loadSoundFromAsset(sound, sound.asset, runtime, soundBank).then((loadedSound: any) => {
        loadedSound.broken = {};
 ...
 * @returns {!Promise} - a promise which will resolve to the sound when ready.
 */
const loadSound = function (sound: any, runtime: any, soundBank: any) {
    if (!runtime.storage) {
